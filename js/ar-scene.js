...
import { createAnimatedFlag, createParticles } from "./effects.js";
...
function createSkyClouds(texture) {
  const group = new THREE.Group();
  const cloudData = [
    { x: -0.78, y: 0.08, z: 0.00, sx: 0.38, sy: 0.17, opacity: 0.38 },
    { x: -0.30, y: 0.16, z: -0.02, sx: 0.30, sy: 0.14, opacity: 0.28 },
    { x: 0.22, y: 0.12, z: -0.01, sx: 0.36, sy: 0.16, opacity: 0.34 },
    { x: 0.72, y: 0.03, z: 0.01, sx: 0.42, sy: 0.18, opacity: 0.32 }
  ];

  cloudData.forEach((item, index) => {
    const material = new THREE.SpriteMaterial({
      map: texture,
      color: index % 2 ? 0xd9efff : 0xffffff,
      transparent: true,
    ...
    sprite.scale.set(item.sx, item.sy, 1);
    sprite.userData.baseX = item.x;
...
function createJetCloudSmoke(texture) {
  const group = new THREE.Group();
  const laneColors = [0xff7a00, 0xffffff, 0x138808];
  const pool = [];
  const perLane = 52;
  ...
  laneColors.forEach((color, lane) => {
    for (let i = 0; i < perLane; i += 1) {
      ...
      sprite.userData = {
        ...
        active: false,
        age: 0,
        lifetime: 3.5 + Math.random() * 1.2,
        drift: (Math.random() - 0.5) * 0.018,
        phase: Math.random() * Math.PI * 2,
        baseSize: 0.045 + Math.random() * 0.018
      };
      pool.push(sprite);
      group.add(sprite);
    }
  ...
  group.userData.cursor = 0;
  group.userData.lastEmit = -100;
  return group;
}

function emitJetClouds(smokeGroup, jetPosition, time) {
  if (!smokeGroup || time - smokeGroup.userData.lastEmit < 0.042) return;
  smokeGroup.userData.lastEmit = time;
  const pool = smokeGroup.userData.pool;

  for (let lane = 0; lane < 3; lane += 1) {
    let sprite = null;
    for (let tries = 0; tries < pool.length; tries += 1) {
      const index = (smokeGroup.userData.cursor + tries) % pool.length;
      if (!pool[index].userData.active && pool[index].userData.lane === lane) {
        sprite = pool[index];
        smokeGroup.userData.cursor = (index + 1) % pool.length;
        break;
      }
    }
    if (!sprite) continue;

    const laneY = (1 - lane) * 0.050;
    sprite.position.set(
      jetPosition.x - 0.14 + (Math.random() - 0.5) * 0.025,
      jetPosition.y + laneY + (Math.random() - 0.5) * 0.016,
      jetPosition.z - 0.015 + (Math.random() - 0.5) * 0.018
    );
    sprite.scale.setScalar(sprite.userData.baseSize);
    sprite.material.opacity = 0.82;
    sprite.material.rotation = Math.random() * Math.PI * 2;
    sprite.userData.age = 0;
    sprite.userData.lifetime = 3.4 + Math.random() * 1.4;
    sprite.userData.active = true;
    sprite.visible = true;
  }
}

function updateJetClouds(smokeGroup, deltaTime, time) {
  if (!smokeGroup) return;
  for (const sprite of smokeGroup.userData.pool) {
    const data = sprite.userData;
    if (!data.active) continue;
    data.age += deltaTime;
    const life = data.age / data.lifetime;
    if (life >= 1) {
      data.active = false;
      sprite.visible = false;
    ...
    sprite.position.x -= deltaTime * (0.045 + life * 0.025);
    sprite.position.y += Math.sin(time * 2.1 + data.phase) * 0.002 + data.drift * deltaTime;
    sprite.position.z += Math.cos(time * 1.7 + data.phase) * 0.0015;

    const expansion = data.baseSize * (1 + life * 2.4);
    sprite.scale.set(expansion * 1.20, expansion, 1);
    const fadeIn = THREE.MathUtils.smoothstep(life, 0, 0.08);
    const fadeOut = 1 - THREE.MathUtils.smoothstep(life, 0.48, 1);
    sprite.material.opacity = 0.82 * fadeIn * fadeOut;
    sprite.material.rotation += deltaTime * data.drift * 8;
  }
}
...
export class ARExperience {
  ...
  async start() {
    ...
    const [stageModel, gateModel, chakraModel, fighterJetModel] = await Promise.all([
      loadModel(loader, CONFIG.models.stage),
      loadModel(loader, CONFIG.models.indiaGate),
      loadModel(loader, CONFIG.models.chakra),
      loadModel(loader, CONFIG.models.fighterJet)
    ]);
    ...
    // Preserve all finalized foreground positions; add the new sky and fly-past behind them.
    const cloudTexture = createCloudTexture();
    const skyClouds = createSkyClouds(cloudTexture);
    applyLayout(skyClouds, SCENE_LAYOUT.skyClouds);
    this.skyClouds = skyClouds;

    const jetSmoke = createJetCloudSmoke(cloudTexture);
    applyLayout(jetSmoke, SCENE_LAYOUT.jetSmoke);
    this.jetSmoke = jetSmoke;

    const fighterJet = new THREE.Group();
    fighterJet.add(normalizeModel(fighterJetModel, {
      rotation: new THREE.Euler(-Math.PI / 2, -Math.PI / 2, 0),
      targetSize: 0.30,
      axis: "width"
    }));
    fighterJet.traverse((object) => {
      if (!object.isMesh) return;
    ...
    // Order matters: clouds and jet first, finalized foreground scene afterward.
    this.contentRoot.add(skyClouds, jetSmoke, fighterJet, flag, videoPanel, greetingPanel, particles);

    anchor.onTargetFound = async () => {
      this.targetVisible = true;
      this.interactionGroup.visible = true;
      this.contentRoot.scale.setScalar(0.025);
      this.jetAnimationStart = performance.now() / 1000;
      if (this.jetSmoke) {
        for (const sprite of this.jetSmoke.userData.pool) {
    ...
    SCENE_LAYOUT.chakra.rotation.z
      ...
      if (this.skyClouds) {
        this.skyClouds.children.forEach((cloud, index) => {
          cloud.position.x = cloud.userData.baseX + Math.sin(time * 0.12 + cloud.userData.phase) * 0.035;
          cloud.material.opacity = (index % 2 ? 0.28 : 0.36) + Math.sin(time * 0.35 + index) * 0.035;
        });
      }

      if (this.fighterJet && this.targetVisible) {
        const flight = SCENE_LAYOUT.fighterJet.flight;
        const totalCycle = flight.duration + flight.pause;
        const localTime = Math.max(0, time - this.jetAnimationStart);
        const cycleTime = localTime % totalCycle;
        const progress = THREE.MathUtils.clamp(cycleTime / flight.duration, 0, 1);
        const isFlying = cycleTime <= flight.duration;

        this.fighterJet.visible = isFlying && progress > 0.015 && progress < 0.985;
        if (isFlying) {
          const eased = THREE.MathUtils.smoothstep(progress, 0, 1);
          this.fighterJet.position.x = THREE.MathUtils.lerp(flight.startX, flight.endX, eased);
          this.fighterJet.position.y = SCENE_LAYOUT.fighterJet.position.y + Math.sin(progress * Math.PI) * flight.verticalArc;
          this.fighterJet.position.z = SCENE_LAYOUT.fighterJet.position.z;
          this.fighterJet.rotation.z = THREE.MathUtils.degToRad(SCENE_LAYOUT.fighterJet.rotation.z) + Math.sin(progress * Math.PI * 2) * 0.055;
          emitJetClouds(this.jetSmoke, this.fighterJet.position, time);
        }
      }
      updateJetClouds(this.jetSmoke, deltaTime, time);
